...
export default class EChartsReactCore extends PureComponent<EChartsReactProps> {
  ...
  componentDidUpdate(prevProps: EChartsReactProps) {
    ...
    if (XTools.isFunction(shouldSetOption) && !shouldSetOption(prevProps, this.props)) {
      return;
    }
    ...
    if (
      !XTools.isEqual(prevProps.theme, this.props.theme) ||
      !XTools.isEqual(prevProps.opts, this.props.opts) ||
      !XTools.isEqual(prevProps.onEvents, this.props.onEvents)
    ) {
      this.dispose();
    ...
    if (!XTools.isEqual(pick(this.props, pickKeys), pick(prevProps, pickKeys))) {
     ...
     */
    if (!XTools.isEqual(prevProps.style, this.props.style) || !XTools.isEqual(prevProps.className, this.props.className)) {
      this.resize();
    }
  }
  ...
  public off(operation){
    this.getEchartsInstance()?.(operation);
  }
  ...
  private renderNewEcharts() {
    ...
    // 3. on chart ready
    if (XTools.isFunction(onChartReady)) onChartReady(echartsInstance);
  ...
  private bindEvents(instance, events: EChartsReactProps['onEvents']) {
    function _bindEvent(eventName: string, func: Function) {
      ...
      if (XTools.isString(eventName) && XTools.isFunction(func)) {
        // binding event
