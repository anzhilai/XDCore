...
import { getScrollTop } from '../../toolkit/utils/dom';
import Drag from '../../layout/drag/Drag';
...
* Rename \`getScrollContainer()\` to \`scrollContainer\` to suppress this warning.`);
  ...
  onScroll = async () => {
    ...
    let ret = await handler();// @ts-ignore
    if (!ret) {
  ...
  onDragEnd = async (_event, { offsetY }) => {
    // 没有产生位移
    if (!offsetY) {
      return;
    }
  ...
  renderRefresh = (): ReactNode => {
    ...
    const { prefixCls, locale = localeZhCn } = this.props;
    ...
    switch (refreshState) {
      ...
      case REFRESH_STATE.success:
        return (
          <div className={cls}>
            {XIcon.SuccessCircle({theme:"success"})}
      ...
      case REFRESH_STATE.failure:
        return (
          <div className={cls}>
            {XIcon.WarningCircle({theme:"danger"})}
