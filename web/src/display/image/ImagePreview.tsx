...
import showOriginButton from './showOriginButton';
...
export default class ImagePreview extends Component<ImagePreviewProps, ImagePreviewState> {
  ...
  renderOriginButton() {
    ...
    const { prefixCls, locale = localeZhCn, activeIndex } = this.props;
      ...
      return (
        <button className={`${prefixCls}__origin__button`} onClick={this.loadOrigin}>
          {loaded === LOAD_STATUS.start && (
            <Loading className={`${prefixCls}__loading`} type="spinner" />
          )}
        ...
        >
          {visible &&
            (images?.length ? (
              ...
              <Loading className={`${prefixCls}__loading`} type="spinner" size="lg" />
            ))}
        </div>
