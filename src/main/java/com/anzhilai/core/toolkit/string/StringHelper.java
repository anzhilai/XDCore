package com.anzhilai.core.toolkit.string;
...
public final class StringHelper {
     ...
     * <li>starts with dialect-specified {@link DBBase#openQuote() open-quote}
     * AND ends with dialect-specified {@link DBBase#closeQuote() close-quote}</li>
     * </ol>
     *
     * @param name The name to check
     * @param db The db (to determine the "real" quoting chars).
     *
     * @return True if quoted, false otherwise
     */
    public static boolean isQuoted(String name, DBBase db) {
        return name != null && name.length() != 0
                && ( ( name.charAt( 0 ) == '`' && name.charAt( name.length() - 1 ) == '`' )
                || ( name.charAt( 0 ) == '"' && name.charAt( name.length() - 1 ) == '"' )
                || ( name.charAt( 0 ) == db.openQuote()
                && name.charAt( name.length() - 1 ) == db.closeQuote() ) );
    }
     ...
     * @return The unquoted version.
     */
    public static String unquote(String name, DBBase db) {
        return isQuoted( name, db ) ? name.substring( 1, name.length() - 1 ) : name;
    }
     ...
     */
    public static String[] unquote(String[] names, DBBase db) {
        if ( names == null ) {
            return null;
        }
        String[] unquoted = new String[names.length];
        for ( int i = 0; i < names.length; i++ ) {
            unquoted[i] = unquote( names[i], db );
        }
        return unquoted;
    }
